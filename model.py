...
def get_model(str_model, cfg):
    ...
    if str_model == 'arch_001':
        ...
        x = Conv3D(32, kernel_size=(20, 20, 1), activation='relu', kernel_initializer='he_uniform')(structure_inputs)
        ...
        x = Conv3D(32, kernel_size=(12, 12, 1), activation='relu', kernel_initializer='he_uniform')(angiography_inputs)
        x = MaxPooling3D(pool_size=(2, 2, 1), strides=2)(x)
        x = BatchNormalization(center=True, scale=True)(x)
        x = Dropout(0.05)(x)
        ...
        x_combined = concatenate([angiography_features, structure_features, bscan_features])
        x = Dense(128, kernel_initializer='he_uniform')(x_combined)
        x = ReLU()(x)
        x = BatchNormalization()(x)
        x = Dropout(0.3)(x)

        x = Dense(64, kernel_initializer='he_uniform')(x_combined)
        x = ReLU()(x)
        x = BatchNormalization()(x)
        x = Dropout(0.3)(x)
        ...
        model.compile(optimizer=Adam(lr=cfg.lr), loss='binary_crossentropy', metrics=['accuracy'])
